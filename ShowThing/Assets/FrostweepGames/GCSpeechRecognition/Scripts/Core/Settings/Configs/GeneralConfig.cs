...
{
    //[CreateAssetMenu(fileName = "GeneralConfig", menuName = "FrostweepGames/GoogleCloud/Translation/GeneralConfig", order = 3)]
    public class GeneralConfig : ScriptableObject
    {
        ...
        {
            string path = "GCTranslation/GeneralConfig";
            var config = Resources.Load<GeneralConfig>(path);

            if(config == null)
            {
                Debug.LogError($"Google Cloud Translation General Config not found in {path} Resources folder. Will use default.");

                config = (GeneralConfig)CreateInstance("GeneralConfig");

#if UNITY_EDITOR
                string pathToFolder = "Assets/FrostweepGames/GCTranslation/Resources/GCTranslation";
